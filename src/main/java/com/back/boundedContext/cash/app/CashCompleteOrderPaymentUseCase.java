...
public class CashCompleteOrderPaymentUseCase {
    ...
    public void handle(OrderDto order, BigDecimal pgPaymentAmount) {
        ...
        if (pgPaymentAmount.compareTo(BigDecimal.ZERO) > 0) {
        ...
        boolean canPay = customerWallet.getBalance().compareTo(order.getSalePrice()) >= 0;
        ...
        if (canPay) {
            ...
            eventPublisher.publisher(
                    new CashOrderPaymentFailedEvent(
                            ...
                            order,
                            pgPaymentAmount,
                            order.getSalePrice().subtract(customerWallet.getBalance())
                    )
            );
        }
    }
